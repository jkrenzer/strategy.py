...
class TimeSpan:
    ...
    def asList(self):
        return [
            value if value is not None else datetime.now() for value in self.asRawList()
        ]
...
class Task(Node):
    ...
    def noDatesSet(self):
        return all([node._manualStart.isEmpty() for node in self.getAll()]) and all(
            [node._manualEnd.isEmpty() for node in self.getAll()]
        )
    ...
    def getManuallyDated(self):
        ...
        return {
            ...
            if not node._manualStart.isEmpty or not node._manualEnd.isEmpty
        }
    ...
    def calculateEarliestStart(self):
        ...
        if self.manualStart._earliest is not None:
            log.debug(
                "Node %s (%s) had a manual earliest start set: %s",
                self.name,
                str(self.getID()),
                str(self.manualStart._earliest),
            )
            return self.manualStart._earliest
        if self.hasParents():
            earliestEndsParents = [
                e for parent in self._parents if (e := parent.calculateEarliestEnd())
            ]
            if earliestEndsParents:
                earliestStart = max(earliestEndsParents)
                log.debug(
                    "Node %s (%s) calculated earliest start: %s",
                    self.name,
                    str(self.getID()),
                    str(earliestStart),
                )
            ...
            log.debug(
                "Node %s (%s) determined earliest start to be latest start",
                self.name,
                str(self.getID()),
            )
    ...
    def calculateLatestEnd(self):
        ...
        if self.manualEnd._latest is not None:
            log.debug(
                "Node %s (%s) had a manual latest end set: %s",
                self.name,
                str(self.getID()),
                str(self.manualEnd._latest),
            )
            return self.manualEnd._latest
        if self.hasChildren():
            latestStartsChildren = [
                s for child in self._children if (s := child.calculateLatestStart())
            ]
            if latestStartsChildren:
                latestEnd = min(latestStartsChildren)
                log.debug(
                    "Node %s (%s) calculated latest end: %s",
                    self.name,
                    str(self.getID()),
                    str(latestEnd),
                )
            ...
            log.debug(
                "Node %s (%s) determined latest end to be earliest end",
                self.name,
                str(self.getID()),
            )
    ...
    def calculateEarliestEnd(self):
        ...
        if self.manualEnd._earliest is not None:
            log.debug(
                "Node %s (%s) had a manual earliest end set: %s",
                self.name,
                str(self.getID()),
                str(self.manualEnd._earliest),
            )
            return self.manualEnd._earliest
        else:
            earliestStart = self.calculateEarliestStart()
            earliestEnd = (
                earliestStart + self.duration if earliestStart is not None else None
            )
            log.debug(
                "Node %s (%s) calculated earliest end: %s",
                self.name,
                str(self.getID()),
                str(earliestEnd),
            )
    ...
    def calculateLatestStart(self):
        ...
        if self.manualStart._latest is not None:
            log.debug(
                "Node %s (%s) had a manual latest start set: %s",
                self.name,
                str(self.getID()),
                str(self.manualStart._latest),
            )
            ...
            log.debug(
                "Node %s (%s) calculated latest start: %s",
                self.name,
                str(self.getID()),
                str(latestStart),
            )
