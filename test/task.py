...
class TestTask(TestCase):
    ...
    def test_complex_example(self):
        ...
        self.assertEqual(g.end.earliest, datetime(year=2018,month=1,day=1,hour=8,minute=0) + timedelta(days=28))
        self.assertEqual(g1.end.earliest, datetime(year=2018,month=1,day=1,hour=8,minute=0) + timedelta(days=17))
        a1.manualStart.earliest = datetime(year=2018,month=2,day=1,hour=8,minute=0)
        self.assertEqual(g.end.earliest, datetime(year=2018,month=2,day=1,hour=8,minute=0) + timedelta(days=23))
        self.assertEqual(g1.end.earliest, datetime(year=2018,month=2,day=1,hour=8,minute=0) + timedelta(days=12))
